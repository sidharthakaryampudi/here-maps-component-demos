...
public class ContextMenu extends PopUpGUItem {
	...
	private int getRowHeight(int i){
		if  (menuItems.getImage(i) != null){
			return Math.max (menuItems.getFont(i).getHeight() , menuItems.getImage(i).getHeight())  + ROW_SPACING;
		}
	...
	public void paint(Graphics g, int x, int y, int anchor) {
		...
		if (menuItems != null) {
			if (menuItems.getLabel() != null) {
				...
				g.fillRoundRect(x, y, getWidth(), Font.getDefaultFont().getHeight()
						+ ROW_SPACING, BackgroundBox.SMALL_CORNER_ARC,
				...
				g.drawString(menuItems.getLabel(), x + TEXT_MARGIN, y + 2,
						anchor);
				y = y + Font.getDefaultFont().getHeight() + ROW_SPACING;
				g.setClip(x,y, getWidth(), getHeight() - Font.getDefaultFont().getHeight() - ROW_SPACING);
			} else {
			...
			for (int i = 0; i < menuItems.size(); i++) {
				...
				if (i == selectedIndex && isHighlight()){
					...
					g.fillRoundRect(x , y - getShift(), getWidth(), getRowHeight(i) , BackgroundBox.SMALL_CORNER_ARC,
							BackgroundBox.SMALL_CORNER_ARC);
					g.setColor(textColor);
				}
				...
				if ( menuItems.getImage(i) != null){
					g.drawImage( menuItems.getImage(i), x, y + 2 - getShift(),	Graphics.TOP | Graphics.LEFT);
				...
				g.drawString(menuItems.getString(i), x + TEXT_MARGIN + imgOff, y + 2 - getShift(),
						anchor);
