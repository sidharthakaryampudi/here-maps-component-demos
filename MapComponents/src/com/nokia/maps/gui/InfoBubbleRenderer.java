...
public class InfoBubbleRenderer extends TooltipRenderer {
	...
	public InfoBubbleRenderer() {
		this(WHITE, SMALL_FONT, new BackgroundBox(DARK_GREY, DARK_GREY, BLACK, DARK_GREY));
	}
	...
	protected void drawTail(Graphics g) {
		g.setColor( isHighlight()  ?  getGUIBackground().getHighlightColor() : getGUIBackground().getBorderColor());
		g.fillRect(anchor.getX() -3, anchor.getY() - 10, 7, 6);
		
		for (int i = 0; i < TAIL.length; i++){
			g.drawLine(anchor.getX() -TAIL[i], anchor.getY() - i, anchor.getX() + TAIL[i] , anchor.getY()-i);
		}
	}
