...
public class RestrictMapComponent implements MapComponent {
     ...
     */
    public RestrictMapComponent(MapListener mapListener, Display display) {
        this(mapListener, display,
                new GeoBoundingBox(
                new GeoCoordinate(MAX_LATITUDE, -MAX_LONGITUDE, Float.NaN),
                new GeoCoordinate(-MAX_LATITUDE, MAX_LONGITUDE, Float.NaN)));
    }
        ...
        if (boundaryTopLeft != null) {
            ...
            if (boundaryTopLeft.getX() > 0) {
                ...
                if (boundaryTopLeft.getX() > NOKIA_LOGO.getX()) {
                    g.fillRect(NOKIA_LOGO.getX(), 0,
                            boundaryTopLeft.getX() - NOKIA_LOGO.getX(),
                            map.getHeight());
                }
            }
            ...
            if (boundaryBottomRight.getX() < map.getWidth()) {
                g.fillRect(boundaryBottomRight.getX(), 0,
                        map.getWidth() - boundaryBottomRight.getX(),
                        map.getHeight());
            }

            if (boundaryBottomRight.getY() < map.getHeight()) {
                int bar = Math.max(boundaryBottomRight.getY(),
                        map.getHeight() - NOKIA_LOGO.getY());

                g.fillRect(NOKIA_LOGO.getX(), bar,
                        map.getWidth() - NOKIA_LOGO.getX(),
                        map.getHeight() - bar);
                if (boundaryBottomRight.getY()
                        < map.getHeight() - NOKIA_LOGO.getY()) {
    ...
    public void attach(MapDisplay map) {
        ...
        if (boundingBox != null) {
            ...
            // screen.
            minZoomLevel = (minZoomLevel < map.getZoomLevel())
                    ? map.getZoomLevel()
                    : minZoomLevel;
        }
    }
